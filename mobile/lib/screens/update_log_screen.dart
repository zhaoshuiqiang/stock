...
class UpdateLogScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final updates = [
      {
        'version': 'v4.28.20260725',
        'date': '2026-07-25',
        'changes': [
          '修复(详情页分时/实时图): 非交易日恒显“暂无分时数据”——分时接口(东财trends2/新浪5分钟)此前严格按当天日期过滤，周末/节假日API返回的上一交易日数据被全部滤掉；现改为交易日只取当天(盘前无数据仍显暂无、不串昨日曲线)，非交易日取最近一个交易日全场，东财/新浪双路径同步修复',
        ],
      },
      {
        'version': 'v4.27.20260724',
        'date': '2026-07-24',
        'changes': [
          '评审修复(详情页筹码): 修正典型价(tp)计算——当日K线 high/low 缺失(为0)时改用回退后的 lo/hi，避免三角分布峰值被错误推到区间边界(仅影响异常数据日)',
        ],
      },
      {
        'version': 'v4.26.20260724',
        'date': '2026-07-24',
        'changes': [
          '优化(全市场扫描): 决策快照生成结果可见——扫描完成后弹出提示“本次扫描已生成N条决策快照/未生成”，此前 unawaited 静默、用户无从感知是否生成',
          '健壮性(全市场扫描): 决策快照捕获改为在报告“完成”前 await 落库(refreshPending/回测等慢网络副作用仍后台执行)，避免App切后台导致 unawaited 捕获中断、快照未生成',
        ],
      },
      {
        'version': 'v4.25.20260724',
        'date': '2026-07-24',
        'changes': [
          '优化(决策页): 1/3/5日校准改为诚实口径——经多轮离线验证(walk-forward + 留档LODO)确认短期方向命中≈随大盘(T+1近掷硬币，条件化regime反而更差、Brier均~0.25)，故不再长期显示“暂无数据”或伪精确命中率，改为提示“方向随大盘 + 当前regime + 相对超额α见留档页”',
          '修复(详情页): 评分趋势不再恒显“推荐记录不足”——数据源由recommendation_tracking(同股20交易日去重、单机几乎攒不够2条)改为decision_snapshots(每次扫描/快照均记录，按交易日去重)，不足2点时回退旧源，向后兼容',
        ],
      },
      {
        'version': 'v4.24.20260724',
        'date': '2026-07-24',
        'changes': [
          '新增(详情页): 筹码分布卡片——基于K线换手衰减+三角分布重建成本分布，展示平均成本(近似主力)、获利盘/套牢盘比例、90%成本区间与集中度、主峰价；纯K线无法区分主力/散户，均值为全体筹码近似',
          '实现: 新增 ChipDistributionAnalyzer(换手率=成交量*100/流通股本，缺失时回退固定扩散率)，含6项TDD单测(边界/去均值/回退)；TradingDashboard新增可折叠筹码卡，获利盘红/套牢盘绿(A股惯例)',
        ],
      },
      {
        'version': 'v4.23.20260724',
        'date': '2026-07-24',
        'changes': [
          '新增(留档页): 相对大盘超额α——顶部方向卡新增“当日留档域行情(均值%与下跌占比)”与“看多/看空/观望超额”三格；以同批留档均值为基准剔除行情影响，衡量剔除普涨普跌后的相对选股能力',
          '说明(留档页): 明确“方向合理率主要反映当日行情”——经11份历史留档+真实指数(akshare)walk-forward验证，当日方向命中率约86%由大盘涨跌决定(普跌日看多命中天然偏低)，超额α才是剔除行情后的真实边际；样本不足3条时不展示',
          '测试: 新增 archive_relative_alpha 单测(单批去均值/多批各自去均值/无效价兜底)；编码安全(无GBK损坏)与版本一致性守卫通过',
        ],
      },
      {
      ...
      },
      {
        ...
        'date': '2026-07-23',
        'changes': [
          '修复(自选页): 评级筛选新增“观望”选项——引擎中性评级(观望)此前不在筛选下拉中，被评为观望的自选股只能在“全部”下出现、无法单独筛选；现下拉在“偏多观望/偏空观望”之间补入“观望”',
          '维护(自选页): 无分析数据时的兜底分类对齐可筛选词表——涨->偏多观望、跌->偏空观望、平->观望，不再产生下拉中缺失的“中性”，使 _classifyStock 输出与筛选项一一对应(category 仅用于筛选、不影响展示与配色)',
        ],
      },
      {
        ...
        'date': '2026-07-23',
        'changes': [
          '分析(全开场景): 梳理13个实验开关全开时的修正叠加——5个抑制趋势/追高开关在信号/技术/共振/方向/实时多层重复扣分，理论上会压制强势股(详见 docs/design_all_on_scoring_coordination.md)',
          '验证结论: 用3281行留档回验——均线多头/趋势强度且温和上涨(0~7%、非抛物线)的目标个股前向收益仍为负(均值约-1.1%~-1.25%、上涨胜率约42%)，与既有“均线多头前向-2.0%”实证一致；即看似被过度压制的强势股实际并未跑赢，上抬回看多带会重新拉高亏损样本',
          '据此撤回: 本版不引入强势趋势护栏，评分/方向行为与验证前字节等价(git diff HEAD~1 已核验)；保留分析文档与验证脚本 scripts/analyze_strong_trend_guard.py，后续如需救援须先用固定1/3/5日outcome定位真正有正edge的子样本并默认关灰度',
          '入库: 一并提交此前未提交的 v4.18 API密钥安全改动(密钥不再随APK打包、改为运行时设置页注入并存本机)',
        ],
      },
      {
      ...
      },
      {
        ...
        'date': '2026-07-22',
        'changes': [
          '修复: 盘中个股详情“分时图”显示昨日走势——分时解析仅按“时:分”映射槽位、丢弃日期，叠加东财 trends 接口未限当日、新浪降级取近5日(datalen=240)，导致昨日/多日数据被错当成今日曲线(未成交时段被昨日填充)；现在数据源层按当日(北京时区)过滤：东财追加 ndays=1 并按日期剔除非今日行，新浪同样按日期剔除，新增 TimeshareParser.dateOf 提取日期',
          '说明: 盘前无当日数据时显示“暂无分时”而非昨日曲线，盘后仍正常显示今日全天；下游30秒轮询合并与解析逻辑不变、回归风险低',
          '测试: 新增分时日期提取(dateOf)用例，全量1164测试通过',
        ],
      },
      {
      ...
      },
      {
        ...
        'date': '2026-07-22',
        'changes': [
          '修复: 决策页面“板块动量”分量刷新后归0——个股详情的手动刷新/实时更新/AI分析三条路径未回传板块数据，导致 sector_momentum(方向证据10%权重)刷新后跌为0、方向分跳变；现缓存首屏板块轮动数据并在三条刷新路径回传，使刷新与首屏口径一致(bug修复)',
          '新增(实验/默认关): 校准小样本参考开关(useCalibrationColdStart)——决策1/3/5日校准严格档需≥100样本/20交易日/95%评估率(单用户长期方达标)，开启后样本不足时分层降级(band级≥40/10日、方向级≥20/8日)显示“校准小样本参考”并附Wilson区间，替代长期“暂无数据”；严格档字节等价、默认关，已接入设置页',
          '测试: 新增校准冷启动5用例(关→空、开→分层估计、严格档优先且isColdStart=false)，全量1162测试通过',
        ],
      },
      {
